...
for cls in ["RF", "AB", "SVM", "DT", "GB", "ET"]:
    ...
    dfs = pd.merge(pd.merge(pd.merge(df1, df2, on='datasets'),
                   df3, on='datasets'), df4, on="datasets")
    dict_cls[cls] = dfs
...
dfs = pd.merge(pd.merge(pd.merge(
    pd.merge(
        pd.merge(
            dict_cls["RF"], dict_cls["GB"], on='datasets'), dict_cls["DT"], on='datasets'), dict_cls["SVM"], on="datasets"), dict_cls["ET"], on="datasets"), dict_cls["AB"], on="datasets")
